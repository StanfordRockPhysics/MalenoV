...
def ex_create(adr_arr,seis_arr,seis_spec,num_examp,cube_incr,cube_step_interval = 1,inp_res=np.float64,sort_adr = False,replace_illegals = True, test_size = 1000):
    ...
    inl_min = inline_start + inline_step*cube_incr*cube_step_interval
    inl_max = inline_end - inline_step*cube_incr*cube_step_interval
    xl_min = xline_start + xline_step*cube_incr*cube_step_interval
    xl_max = xline_end - xline_step*cube_incr*cube_step_interval
    t_min = t_start + t_step*cube_incr*cube_step_interval
    t_max = t_end - t_step*cube_incr*cube_step_interval
    ...
    print('Number of labels:', len(adr_arr))
    n=0
    for i in range(num_examp):
        ...
        for j in range(50):
            ...
            if (adr[0]>=inl_min and adr[0]<inl_max) and \
                ...
                examples[i-n,:,:,:,:] = seis_arr[idx[0]- cube_incr* cube_step_interval:idx[0]+cube_step_interval*cube_incr+1:cube_step_interval,\
                    idx[1]-cube_step_interval*cube_incr:idx[1]+cube_step_interval*cube_incr+1:cube_step_interval,\
                    idx[2]-cube_step_interval*cube_incr:idx[2]+cube_step_interval*cube_incr+1:cube_step_interval, :]
    ...
    return (examples[0:num_examp-n-test_size,:,:,:,:], labels[0:num_examp-n-test_size], examples[num_examp-n-test_size:, :,:,:,:], labels[num_examp-n-test_size:])
...
def train_model(segy_obj,class_array,num_classes,cube_incr,inp_res = np.float64,\
    ...
    if keras_model == None:
        ...
        model.add(Conv3D(50, (5, 5, 5), padding='same', input_shape=(cube_size,cube_size,cube_size,num_channels), strides=(4, 4, 4), data_format="channels_last",name = 'conv_layer1'))
        model.add(BatchNormalization())
        model.add(Activation('relu'))
    ...
    for i in range(num_bunch):
        ...
        (x_train, y_train, x_test, y_test) = ex_create(adr_arr = class_array,
                                       seis_arr = segy_obj.data,
                                       seis_spec = segy_obj,
                                       num_examp = num_examples,
                                       cube_incr = cube_incr,
                                       cube_step_interval = sample_step,
                                       inp_res = inp_res,
                                       sort_adr = False,
        ...
        if not data_augmentation:
            ...
            thefile = open(write_location+'_acc.txt', 'w')
            for item in history.acc:
                thefile.write("%f\n" % item)

            thefile = open(write_location+'_loss.txt', 'w')
        ...
        print('Test set accuracy:', np.sum(pred_test==y_test)/y_test.shape[0])
...
def cube_parse(seis_arr,cube_incr, cube_step_interval = 1,inp_res = np.float64, mode = 'trace', padding = False,\
    ...
    if padding:
        ...
        i_re = cube_incr*cube_step_interval
        x_re = cube_incr*cube_step_interval
        z_re = cube_incr*cube_step_interval
        # Preallocate the output array, if concatenated it's 5 dimensional, if not it's 7 dimensional
        if conc:
            # Make adjustments to the parameters so that we iterate over the right number of samples, etc.
            if mode == 'full':
                examples = np.empty(((inls-2*cube_incr*cube_step_interval)*(xls-2*cube_incr*cube_step_interval)*(zls-2*cube_incr*cube_step_interval),cube_size,cube_size,cube_size,num_channels),\
                                   dtype=inp_res)
            elif mode == 'inline':
                examples = np.empty(((xls-2*cube_incr*cube_step_interval)*(zls-2*cube_incr*cube_step_interval),cube_size,cube_size,cube_size,num_channels),\
                                   dtype=inp_res)
                inline_num -= cube_incr*cube_step_interval
                xline_num = 0
                depth = 0
            elif mode == 'xline':
                examples = np.empty(((inls-2*cube_incr*cube_step_interval)*(zls-2*cube_incr*cube_step_interval),cube_size1,cube_size,cube_size,num_channels),\
                                   dtype=inp_res)
                inline_num = 0
                xline_num -= cube_incr*cube_step_interval
                depth = 0
            elif mode == 'trace':
                examples = np.empty((zls-2*cube_incr*cube_step_interval,cube_size,cube_size,cube_size,num_channels),dtype=inp_res)
                inline_num -= cube_incr*cube_step_interval
                xline_num -= cube_incr*cube_step_interval
                depth = 0
            elif mode == 'point':
                ...
                inline_num -= cube_incr*cube_step_interval
                xline_num -= cube_incr*cube_step_interval
                depth -= cube_incr*cube_step_interval
            else:
                print('ERROR: invalid mode! use: ''full'',''xline'',''inline'',''trace'', or ''point''')
            # Take into account that we will have a total smaller dimensionality of data due to illegals
            inls -= 2*cube_incr*cube_step_interval
            xls -= 2*cube_incr*cube_step_interval
            zls -= 2*cube_incr*cube_step_interval
        else:
            if mode == 'full':
                examples = np.empty(((inls-2*cube_incr*cube_step_interval),(xls-2*cube_incr*cube_step_interval),(zls-2*cube_incr*cube_step_interval),cube_size,cube_size,cube_size,num_channels),\
                                   dtype=inp_res)
            elif mode == 'inline':
                examples = np.empty((1,(xls-2*cube_incr*cube_step_interval),(zls-2*cube_incr*cube_step_interval),cube_size,cube_size,cube_size,num_channels),\
                                   dtype=inp_res)
            elif mode == 'xline':
                examples = np.empty(((inls-2*cube_incr*cube_step_interval),1,(zls-2*cube_incr*cube_step_interval),cube_size,cube_size,cube_size,num_channels),\
                                   dtype=inp_res)
            elif mode == 'trace':
                examples = np.empty((1,1,(zls-2*cube_incr*cube_step_interval),cube_size,cube_size,cube_size,num_channels),dtype=inp_res)
            elif mode == 'point':
    ...
    if conc:
        # Make the cubes
        for i in range(cube_incr*cube_step_interval, inls+cube_incr*cube_step_interval):
            if mode == 'xline':
                j = xline_num
                for k in range(cube_incr*cube_step_interval, zls+cube_incr*cube_step_interval):
                    examples[inls*(i-i_re)+k-z_re,:,:,:,:] = seis_arr[i-cube_incr*cube_step_interval+inline_num:i+cube_incr*cube_step_interval+inline_num+1:cube_step_interval,\
                                                                    j-cube_incr*cube_step_interval:j+cube_incr*cube_step_interval+1:cube_step_interval,\
                                                                    k-cube_incr*cube_step_interval+depth:k+cube_incr*cube_step_interval+depth+1:cube_step_interval,:]
            else:
                for j in range(cube_incr*cube_step_interval, xls+cube_incr*cube_step_interval):
                    for k in range(cube_incr*cube_step_interval, zls+cube_incr*cube_step_interval):
                        examples[(i-i_re)*inls+(j-x_re)*xls+k-z_re,:,:,:,:] = \
                                                            seis_arr[i-cube_incr*cube_step_interval+inline_num:i+cube_incr*cube_step_interval+inline_num+1:cube_step_interval,\
                                                                     j-cube_incr*cube_step_interval+xline_num:j+cube_incr*cube_step_interval+xline_num+1:cube_step_interval,\
                                                                     k-cube_incr*cube_step_interval+depth:k+cube_incr*cube_step_interval+depth+1:cube_step_interval,:]
        ...
        for i in range(cube_incr*cube_step_interval, inls-cube_incr*cube_step_interval):
            if mode == 'xline':
                for k in range(cube_incr*cube_step_interval, zls-cube_incr*cube_step_interval):
                    examples[i-i_re,1,k-z_re,:,:,:,:] = seis_arr[i-cube_incr*cube_step_interval:i+cube_incr*cube_step_interval+1:cube_step_interval,\
                                                               xline_num-cube_incr*cube_step_interval:xline_num+cube_incr*cube_step_interval+1:cube_step_interval,\
                                                               k-cube_incr*cube_step_interval:k+cube_incr*cube_step_interval+1:cube_step_interval,:]
            else:
                for j in range(cube_incr*cube_step_interval, xls-cube_incr*cube_step_interval):
                    for k in range(cube_incr*cube_step_interval, zls-cube_incr*cube_step_interval):
                        examples[i-i_re,j-x_re,k-z_re,:,:,:,:] = seis_arr[i+inline_num-cube_incr*cube_step_interval:i+inline_num+cube_incr*cube_step_interval+1:cube_step_interval,\
                                                                        j+xline_num-cube_incr*cube_step_interval:j+xline_num+cube_incr*cube_step_interval+1:cube_step_interval,\
                                                                        k+depth-cube_incr*cube_step_interval:k+depth+cube_incr*cube_step_interval+1:cube_step_interval,:]
...
def predicting(filename,inp_seis,seis_obj,keras_model,cube_incr,num_classes,cube_step_interval = 1,inp_res=np.float64,mode='xline',\
    ...
    if mode == 'inline':
        section_edge = np.asarray([line_num,line_num,cube_incr*cube_step_interval,xls-cube_incr*cube_step_interval,cube_incr*cube_step_interval,zls-cube_incr*cube_step_interval])
    elif mode == 'xline':
        section_edge = np.asarray([cube_incr*cube_step_interval,inls-cube_incr*cube_step_interval,line_num,line_num,cube_incr*cube_step_interval,zls-cube_incr*cube_step_interval])
    elif mode == 'section':
        section_edge = section
    elif mode == 'full':
        section_edge = np.asarray([cube_incr*cube_step_interval,inls-cube_incr*cube_step_interval,cube_incr*cube_step_interval,xls-cube_incr*cube_step_interval,cube_incr*cube_step_interval,zls-cube_incr*cube_step_interval])
    else:
...
def visualization(filename,inp_seis,seis_obj,keras_model,cube_incr,section_edge, plot_type, plot_ref, num_classes,cube_step_interval = 1,\
    ...
    if sect_form == 'segy':
        ...
        if plot_type == 'xline':
            plot_ref = (plot_ref - seis_obj.xl_start)//seis_obj.xl_step
        if plot_type == 'inline':
            plot_ref = (plot_ref - seis_obj.inl_start)//seis_obj.inl_step
            print("Inline start and step: ", seis_obj.inl_start, seis_obj.inl_step)
        if plot_type == 'time':
            plot_ref = (plot_ref - seis_obj.t_start)//seis_obj.t_step
    ...
    if show_feature:
        ...
        plt.title('x-line')
        plt.imshow(inp_seis[cube_incr*cube_step_interval:-cube_incr*cube_step_interval,xline_ref,cube_incr*cube_step_interval:-cube_incr*cube_step_interval].T,interpolation="nearest",\
                   cmap="gray",extent=[cube_incr*cube_step_interval,-cube_incr*cube_step_interval+len(inp_seis),cube_incr*cube_step_interval-len(inp_seis[0,0]),-cube_incr*cube_step_interval])
        ...
        if plot_type == 'xline':
            plt.imshow(np.squeeze(inp_seis[cube_incr:-cube_incr,plot_ref,cube_incr:-cube_incr].T),interpolation="nearest",\
                   cmap="gray",extent=[cube_incr,-cube_incr+len(inp_seis),cube_incr-len(inp_seis[0,0]),-cube_incr])
        if plot_type == 'inline':
            plt.imshow(np.squeeze(inp_seis[plot_ref, cube_incr:-cube_incr, cube_incr:-cube_incr].T),interpolation="nearest",\
                   cmap="gray",extent=[cube_incr,-cube_incr+len(inp_seis[0]),cube_incr-len(inp_seis[0,0]),-cube_incr])
        if plot_type == 'time':
            plt.imshow(np.squeeze(inp_seis[cube_incr:-cube_incr,cube_incr:-cube_incr,plot_ref].T),interpolation="nearest",\
                   cmap="gray",extent=[cube_incr,-cube_incr+len(inp_seis),cube_incr-len(inp_seis[0]),-cube_incr])


        #plt.title('x-line'+str(xline_ref))
        #plt.imshow(np.squeeze(inp_seis[cube_incr:-cube_incr,xline_ref,cube_incr:-cube_incr].T),interpolation="nearest",\
        #           cmap="gray",extent=[cube_incr,-cube_incr+len(inp_seis),cube_incr-len(inp_seis[0,0]),-cube_incr])
        ...
        if plot_type == 'xline':
            plt.imshow(pred[:,plot_ref-section_edge[2],:,class_row].T,interpolation="nearest", cmap="gist_rainbow", clim=(0.0, c_max),\
                  extent=[section_edge[0],section_edge[1],-section_edge[5],-section_edge[4]])
        if plot_type == 'inline':
            plt.imshow(pred[plot_ref-section_edge[0],:,:,class_row].T,interpolation="nearest", cmap="gist_rainbow", clim=(0.0, c_max),\
                  extent=[section_edge[2],section_edge[3],-section_edge[5],-section_edge[4]])
        if plot_type == 'time':
            plt.imshow(pred[:,:,plot_ref-section_edge[4],class_row].T,interpolation="nearest", cmap="gist_rainbow", clim=(0.0, c_max),\
                  extent=[section_edge[0],section_edge[1],-section_edge[3],-section_edge[2]])
...
def show_details(filename,cube_incr,predic,inline,inl_start,xline,xl_start,\
                 slice_number,slice_incr,cube_step_interval = 1,inp_format=np.float64,show_prob = True,num_classes = 2):
    # filename: filename of the segy-cube to be imported (necessary for copying the segy-frame before writing a new segy)
    # cube_incr: number of increments included in each direction from the example to make a mini-cube
    ...
    plt.imshow(inp_seis[inline-cube_incr*cube_step_interval:inline+cube_incr*cube_step_interval,xline,cube_incr*cube_step_interval:-cube_incr*cube_step_interval].T,interpolation="nearest", cmap="gray")
    plt.colorbar()
    ...
    plt.imshow(inp_seis[inline,xline-cube_incr*cube_step_interval:xline+cube_incr*cube_step_interval,cube_incr*cube_step_interval:-cube_incr*cube_step_interval].T,interpolation="nearest", cmap="gray")
    plt.colorbar()
    ...
    plt.imshow(inp_seis[inline-cube_incr*cube_step_interval:inline+cube_incr*cube_step_interval,xline-cube_incr*cube_step_interval:xline+cube_incr*cube_step_interval,cube_incr*cube_step_interval+slice_number-slice_incr].T,\
               interpolation="nearest", cmap="gray")

    plt.subplot(1, 3, 2)
    plt.title('slice: ' + str(slice_number))
    plt.imshow(inp_seis[inline-cube_incr*cube_step_interval:inline+cube_incr*cube_step_interval,xline-cube_incr*cube_step_interval:xline+cube_incr*cube_step_interval,cube_incr*cube_step_interval+slice_number].T,\
               interpolation="nearest", cmap="gray")
...
def master(segy_filename,inp_format,cube_incr,train_dict={},pred_dict={},mode = 'full'):
    ...
    if mode == 'train' or mode == 'full':
        ...
        test_size = train_dict['test_size']
        sample_step = train_dict['sample_step']
    ...
    if mode == 'predict' or mode == 'full':
        ...
        sample_step = pred_dict['sample_step']
        plot_type = pred_dict['plot_type']
...
train_dict = {
    ...
    'save_location' : 'nold_2e_50000_16_1',   # file name for the saved trained model
    'test_size': 2000,
    'sample_step': 1
}
...
pred_dict = {
    ...
    'plot_type': 'inline',
    'plot_ref' : 339,    # xline used for classification (index)(should be within section range)
    ...
    'pred_batch' : 32,     # number of traces used to make batches of mini-cubes that are stored in memory at once
