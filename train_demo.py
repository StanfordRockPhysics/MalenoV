...
def train_model(segy_obj,class_array,num_classes,cube_incr, cube_step_interval = 1,inp_res = np.float64,\
    ...
    for i in range(num_bunch):
        ...
        thefile = open(write_location+'_pred.txt', 'w')
        for item in pred_test:
            thefile.write("%f\n" % item)
        thefile = open(write_location+'_true.txt', 'w')
        for item in y_test:
            thefile.write("%f\n" % item)
...
def visualization(filename,inp_seis,seis_obj,keras_model,cube_incr,section_edge,xline_ref,num_classes,cube_step_interval = 1,\
...
def master(segy_filename,inp_format,cube_incr,cube_step_interval = 1,train_dict={},pred_dict={},mode = 'full'):
